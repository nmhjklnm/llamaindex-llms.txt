...
`embed_batch_size` |  `int` |  The batch size used for the computation. Defaults to DEFAULT_EMBED_BATCH_SIZE. |  `DEFAULT_EMBED_BATCH_SIZE`  
...
`num_workers` |  `int` |  The number of workers to use for async embedding calls. Defaults to None. |  _required_  
...
Name | Type | Description | Default  
---|---|---|---  
`task` |  `str` |  Hugging Face task to check within. A list of all tasks can be found here: https://huggingface.co/tasks |  _required_  
Source code in `llama-index-integrations/embeddings/llama-index-embeddings-huggingface/llama_index/embeddings/huggingface/base.py`
