...
---|---|---|---  
`response` |  `str` |  |  `''`  
`sources` |  `List[ToolOutput]` |  Built-in mutable sequence. If no argument is given, the constructor creates a new empty list. The argument must be an iterable if specified. |  `<dynamic>`  
`source_nodes` |  `List[NodeWithScore]` |  Built-in mutable sequence. If no argument is given, the constructor creates a new empty list. The argument must be an iterable if specified. |  `<dynamic>`  
...
`is_function_not_none_thread_event` |  `Event` |  Class implementing event objects. Events manage a flag that can be set to true with the set() method and reset to false with the clear() method. The wait() method blocks until the flag is true. The flag is initially false. |  `<threading.Event at 0x7059fec02570: unset>`  
