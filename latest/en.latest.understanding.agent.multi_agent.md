...
When more than one specialist is required to solve a task you have several options in LlamaIndex, each trading off convenience for flexibility. This page walks through the three most common patterns, when to choose each one, and provides a minimal code sketch for every approach.
  1. **AgentWorkflow (built-in)** – declare a set of agents and let `AgentWorkflow` manage the hand-offs. Section Full Notebook
  2. **Orchestrator pattern (built-in)** – an "orchestrator" agent chooses which sub-agent to call next; those sub-agents are exposed to it as **tools**. Section Full Notebook
  3. **Custom planner (DIY)** – you write the LLM prompt (often XML / JSON) that plans the sequence yourself and imperatively invoke the agents in code. Section Full Notebook
...
`AgentWorkflow` is itself a Workflow pre-configured to understand agents, state and tool-calling. You supply an _array_ of one or more agents, tell it which one should start, and it will:
...
You can see the full example in the agents_as_tools notebook.
```
import re
...
Below is a _minimal_ sketch of a workflow that can plan, execute a plan, and see if any further steps are needed. You can see the full example in the custom_multi_agent notebook.
```
import re
