...
---|---|---|---  
`project_id` |  `str` |  str |  _required_  
`data_store_id` |  `str` |  str |  _required_  
`location_id` |  `str` |  str = "global" |  `'global'`  
`serving_config_id` |  `str` |  str = "default_config" |  `'default_config'`  
`credentials` |  `Any` |  Any = None |  `None`  
`engine_data_type` |  `int` |  int = 0 |  `0`  
Example
...
Filter expression.
###  get_extractive_answers `class-attribute` `instance-attribute` #
```
get_extractive_answers: bool = False
...
If True return Extractive Answers, otherwise return Extractive Segments or Snippets.
###  max_documents `class-attribute` `instance-attribute` #
```
...
The maximum number of documents to return.
###  max_extractive_answer_count `class-attribute` `instance-attribute` #
```
max_extractive_answer_count: int = 1
...
###  max_extractive_segment_count `class-attribute` `instance-attribute` #
```
max_extractive_segment_count: int = 1
...
###  query_expansion_condition `class-attribute` `instance-attribute` #
```
query_expansion_condition: int = 1
...
###  spell_correction_mode `class-attribute` `instance-attribute` #
```
spell_correction_mode: int = 1
...
###  boost_spec `class-attribute` `instance-attribute` #
```
boost_spec: Optional[Dict[Any, Any]] = None
...
###  return_extractive_segment_score `class-attribute` `instance-attribute` #
```
return_extractive_segment_score: bool = True
