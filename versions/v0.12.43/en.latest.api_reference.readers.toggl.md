...
###  load_data #
```
load_data(workspace_id: str, project_id: str, start_date: Optional[datetime] = None, end_date: Optional[datetime] = now(), out_format: TogglOutFormat = json) -> List[Document]
...
---|---|---|---  
`workspace_id` |  `str` |  The workspace ID. |  _required_  
`project_id` |  `str` |  The project ID. |  _required_  
`start_date` |  `Optional[datetime]` |  The start date. |  `None`  
`end_date` |  `Optional[datetime]` |  The end date. |  `now()`  
`out_format` |  `TogglOutFormat` |  The output format. |  `json`  
Source code in `llama-index-integrations/readers/llama-index-readers-toggl/llama_index/readers/toggl/base.py`
