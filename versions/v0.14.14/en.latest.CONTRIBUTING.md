...
* * *
## 🎯 Quick Start Guide#
We use `uv` as the package and project manager for all the Python packages in this repository. Before contributing, make sure you have `uv` installed. On macOS and Linux:
```
...
```
  ...
  1. Navigate to the project folder you want to work on. For example, if you want to work on the OpenAI llm integration:
...
```
  ...
  1. `uv` will take care of creating and setting up the virtual environment for the specific project you're working on. For example, to run the tests you can do:
...
```
  ...
  1. If you want to create the virtual environment explicitly, without `uv run`ning a command:
...
```
  ...
  4. **Set up your environment** (follow the Quick Start Guide).
...
LlamaIndex is organized as a **monorepo** , meaning different packages live within this single repository. You can focus on a specific package depending on your contribution:
  ...
  * **Integrations** : e.g., `llama-index-integrations/`
