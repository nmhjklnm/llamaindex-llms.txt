...
`host` |  `str` |  Specifies the hostname, IP address, or URL for the database connection. The default scheme is "mysql". |  _required_  
`user` |  `str` |  Database username. |  _required_  
`password` |  `str` |  Database password. |  _required_  
`port` |  `int` |  Database port. Defaults to 3306 for non-HTTP connections, 80 for HTTP connections, and 443 for HTTPS connections. |  _required_  
`database` |  `str` |  Database name. |  _required_  
Examples:
...
Name | Type | Description | Default  
---|---|---|---  
`query` |  `VectorStoreQuery` |  Contains query_embedding and similarity_top_k attributes. |  _required_  
