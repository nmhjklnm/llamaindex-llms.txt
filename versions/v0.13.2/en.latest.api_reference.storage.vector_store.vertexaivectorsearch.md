...
---|---|---|---  
`project_id` |  `str) ` |  The Google Cloud Project ID. |  `None`  
`region` |  `str)     ` |  The default location making the API calls. It must be the same location as where Vector Search index created and must be regional. |  `None`  
`index_id` |  `str)   ` |  The fully qualified resource name of the created index in Vertex AI Vector Search. |  `None`  
`endpoint_id` |  `str` |  The fully qualified resource name of the created index endpoint in Vertex AI Vector Search. |  `None`  
