...
# set api key in env or in llm # get free API key from: https://cleanlab.ai/ # import os # os.environ["CLEANLAB_API_KEY"] = "your api key" llm = CleanlabTLM(api_key="your_api_key")
In [ ]:
Copied!
```
...
options = { "model": "gpt-4", "max_tokens": 128, } llm = CleanlabTLM(api_key="your_api_key", options=options)
In [ ]:
Copied!
```
resp = llm.complete("Who is Paul Graham?")
...
options = { "log": ["explanation"], } llm = CleanlabTLM(api_key="your_api_key", options=options) resp = llm.complete( "What was the horsepower of the first automobile engine used in a commercial truck in the United States?" )
In [ ]:
Copied!
```
print(resp)
