...
`deployment_target` |  `str` |  Deployment target, either `local` or `cloud` |  `'local'`  
`port` |  `int` |  Port that Vespa application will run on. Only applicable if deployment_target is `local` |  `8080`  
`default_schema_name` |  `str` |  Schema name in Vespa application |  `'doc'`  
`namespace` |  `str` |  Namespace in Vespa application. See https://docs.vespa.ai/en/documents.html#namespace. Defaults to `default`. |  `'default'`  
...
Name | Type | Description | Default  
---|---|---|---  
`nodes` |  `List[BaseNode]` |  List of nodes to add |  _required_  
...
`nodes` |  `List[BaseNode]` |  List of nodes to add |  _required_  
`schema` |  `Optional[str]` |  Schema name in Vespa application to add nodes to. Defaults to `default_schema_name`. |  `None`  
`max_connections` |  `int` |  Maximum number of connections to Vespa application |  `10`  
`num_concurrent_requests` |  `int` |  Maximum number of concurrent requests |  `1000`  
`total_timeout` |  `int` |  Total timeout for all requests |  `60`  
`kwargs` |  `Any` |  Additional kwargs for Vespa application |  `{}`  
Source code in `llama-index-integrations/vector_stores/llama-index-vector-stores-vespa/llama_index/vector_stores/vespa/base.py`
