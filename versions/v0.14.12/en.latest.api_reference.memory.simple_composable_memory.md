...
---|---|---|---  
`primary_memory` |  `BaseMemory` |  (BaseMemory) The main memory buffer for agent. |  _required_  
`secondary_memory_sources` |  `List[Annotated[BaseMemory, SerializeAsAny]]` |  (List(BaseMemory)) Secondary memory sources. Retrieved messages from these sources get added to the system prompt message. |  `<dynamic>`  
