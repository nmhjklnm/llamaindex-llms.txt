...
###  add #
```
add(nodes: List[BaseNode], *, rebuild_index: bool = True, rebuild_timeout: Optional[int] = None, **add_kwargs: Any) -> List[str]
...
###  async_add `async` #
```
async_add(nodes: List[BaseNode], *, rebuild_index: bool = True, rebuild_timeout: Optional[int] = None, **add_kwargs: Any) -> List[str]
...
---|---|---  
`VectorStoreQueryResult` |  `VectorStoreQueryResult` |  Query result containing nodes, similarities, and ids.  
Source code in `llama-index-integrations/vector_stores/llama-index-vector-stores-baiduvectordb/llama_index/vector_stores/baiduvectordb/base.py`
