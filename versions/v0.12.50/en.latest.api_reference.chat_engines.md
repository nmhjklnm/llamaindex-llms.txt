...
`is_function_not_none_thread_event` |  `Event` |  Class implementing event objects. Events manage a flag that can be set to true with the set() method and reset to false with the clear() method. The wait() method blocks until the flag is true. The flag is initially false. |  `<threading.Event at 0x76e0c02deab0: unset>`  
