...
`model_name` |  `str` |  The name of the model to be used for generating embeddings. The class ensures that this model is supported. Defaults to "general:embedding". |  `'general:embedding'`  
`embed_batch_size` |  `int` |  The batch size for embedding. Defaults to DEFAULT_EMBED_BATCH_SIZE. |  `DEFAULT_EMBED_BATCH_SIZE`  
