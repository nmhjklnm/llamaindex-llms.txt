...
---|---|---|---  
`limit` |  `Optional[int]` |  limit the number of returned messages to a maximum. |  `None`  
`filter` |  `Optional[Callable[[List[ChatMessage]], List[ChatMessage]]]` |  a function that filters the list of messages to return only the ones that respect certain conditions. |  `None`  
Source code in `llama-index-integrations/voice_agents/llama-index-voice-agents-elevenlabs/llama_index/voice_agents/elevenlabs/base.py`
