...
```
from_defaults(chunk_size: int = 1200, window_size: int = 11, llm_workers: int = 1, llm: Optional[LLM] = None, token_counter: Optional[TokenCounter] = None, sentence_splitter: Optional[Callable[[str], List[str]]] = None, callback_manager: Optional[CallbackManager] = None, id_func: Optional[Callable[[int, Document], str]] = None) -> SlideNodeParser
...
---|---  
###  create_individual_chunks #
```
create_individual_chunks(sentences: List[str]) -> List[str]
...
```
build_localised_splits(chunks: List[str]) -> List[Dict[str, str]]
...
---|---  
###  abuild_localised_splits `async` #
```
abuild_localised_splits(chunks: List[str], show_progress: bool = False) -> List[Dict[str, str]]
...
###  post_process_nodes #
```
post_process_nodes(nodes: List[BaseNode], contexts: List[str]) -> List[BaseNode]
...
```
build_slide_nodes_from_documents(documents: Sequence[Document]) -> List[BaseNode]
...
---|---  
###  abuild_slide_nodes_from_documents `async` #
```
abuild_slide_nodes_from_documents(documents: Sequence[Document], show_progress: bool = False) -> List[BaseNode]
