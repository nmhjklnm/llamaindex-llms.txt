...
---|---|---|---  
`elapsed_time` |  `float` |  Time in seconds that passed since the last attempt. |  _required_  
`attempts` |  `int` |  The number of attempts done so far. |  _required_  
`error` |  `Exception` |  The last error occurred. |  _required_  
Returns:
Type | Description  
---|---  
`float | None` |  The amount of seconds to wait before the next attempt, or None if we stop retrying.  
Source code in `workflows/retry_policy.py`
