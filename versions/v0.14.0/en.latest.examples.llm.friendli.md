...
from llama_index.llms.friendli import Friendli # To customize your friendli token, do this # otherwise it will lookup FRIENDLI_TOKEN from your env variable # llm = Friendli(friendli_token="Your personal access token") llm = Friendli()
...
from llama_index.core.llms import ChatMessage, MessageRole message = ChatMessage(role=MessageRole.USER, content="Tell me a joke.") resp = llm.chat([message]) print(resp)
```
...
prompt = "Draft a cover letter for a role in software engineering." resp = llm.complete(prompt) print(resp)
```
