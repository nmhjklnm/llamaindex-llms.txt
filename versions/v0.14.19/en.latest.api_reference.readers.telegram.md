...
---|---|---|---  
`session_name` |  `str` |  The file name of the session file to be used if a string is given (it may be a full path), or the Session instance to be used otherwise. |  _required_  
`api_id` |  `int` |  The API ID you obtained from https://my.telegram.org. |  _required_  
`api_hash` |  `str` |  The API hash you obtained from https://my.telegram.org. |  _required_  
`phone_number` |  `str` |  The phone to which the code will be sent. |  _required_  
...
###  load_data #
```
load_data(entity_name: str, post_id: Optional[int] = None, limit: Optional[int] = None, start_date: Optional[datetime] = None, end_date: Optional[datetime] = None) -> List[Document]
...
Name | Type | Description | Default  
---|---|---|---  
`entity_name` |  `str` |  The entity from whom to retrieve the message history. |  _required_  
`post_id` |  `int` |  If set to a post ID, the comments that reply to this ID will be returned. Else will get posts/chat messages. |  `None`  
`limit` |  `int` |  Number of messages to be retrieved. |  `None`  
