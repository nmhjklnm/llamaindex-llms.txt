...
---|---|---|---  
`name` |  `str` |  Unique name of the ingestion pipeline. Defaults to DEFAULT_PIPELINE_NAME. |  `'default'`  
`project_name` |  `str` |  Unique name of the project. Defaults to DEFAULT_PROJECT_NAME. |  `'Default'`  
`transformations` |  `List[TransformComponent]` |  Transformations to apply to the data. Defaults to None. |  _required_  
`documents` |  `Optional[Sequence[Document]]` |  Documents to ingest. Defaults to None. |  _required_  
`readers` |  `Optional[List[ReaderConfig]]` |  Reader to use to read the data. Defaults to None. |  _required_  
`vector_store` |  `Optional[BasePydanticVectorStore]` |  Vector store to use to store the data. Defaults to None. |  _required_  
...
`docstore_strategy` |  `DocstoreStrategy` |  Document de-dup strategy. Defaults to DocstoreStrategy.UPSERTS. |  `<DocstoreStrategy.UPSERTS: 'upserts'>`  
`disable_cache` |  `bool` |  Disable the cache. Defaults to False. |  `False`  
`base_url` |  `str` |  Base URL for the LlamaCloud API. Defaults to DEFAULT_BASE_URL. |  _required_  
`app_url` |  `str` |  Base URL for the LlamaCloud app. Defaults to DEFAULT_APP_URL. |  _required_  
