...
---|---|---|---  
`table_name` |  `str` |  The name of the preexisting DynamoDB table. |  _required_  
`primary_key` |  `str` |  The primary/partition key to use for the table. Defaults to "SessionId". |  `'SessionId'`  
`profile_name` |  `str` |  The AWS profile to use. If not specified, then the default AWS profile is used. |  `None`  
`aws_access_key_id` |  `str` |  The AWS Access Key ID to use. |  `None`  
`aws_secret_access_key` |  `str` |  The AWS Secret Access Key to use. |  `None`  
`aws_session_token` |  `str` |  The AWS Session Token to use. |  `None`  
`botocore_session` |  `Any` |  Use this Botocore session instead of creating a new default one. |  `None`  
`botocore_config` |  `Any` |  Custom configuration object to use instead of the default generated one. |  `None`  
`region_name` |  `str` |  The AWS region name to use. Uses the region configured in AWS CLI if not passed. |  `None`  
`max_retries` |  `int` |  The maximum number of API retries. Defaults to 10. |  `10`  
...
`ttl_attribute` |  `str` |  The name of the attribute to use for TTL. Defaults to "TTL". |  `'TTL'`  
Returns:
