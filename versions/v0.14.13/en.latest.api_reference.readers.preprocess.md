...
---|---|---|---  
`api_key` |  `str` |  [Required] The Preprocess API Key. If you don't have one yet, please request it at support@preprocess.co. Default: `None` |  _required_  
`file_path` |  `str` |  [Required] The path to the document to be preprocessed (convertend and split into chunks). Default: `None` |  _required_  
`table_output_format` |  `str` |  The output format for tables within the document. Accepted values are [text, markdown, html]. Default: `text` |  _required_  
...
Type | Description  
---|---  
`List[Document]` |  List[Document]: A list of documents each document containing a chunk from the original document.  
Source code in `llama-index-integrations/readers/llama-index-readers-preprocess/llama_index/readers/preprocess/base.py`
...
Type | Description  
---|---  
`List[TextNode]` |  List[TextNode]: List of nodes, each node will contains a chunk from the original document.  
Source code in `llama-index-integrations/readers/llama-index-readers-preprocess/llama_index/readers/preprocess/base.py`
