...
from llama_index.llms.alephalpha import AlephAlpha # To customize your token, do this # otherwise it will lookup AA_TOKEN from your env variable # llm = AlephAlpha(token="") llm = AlephAlpha(model="luminous-base-control") resp = llm.complete("Paul Graham is ")
In [ ]:
Copied!
```
print(resp)
...
from llama_index.llms.alephalpha import AlephAlpha llm = AlephAlpha(model="luminous-base-control", log_probs=0) resp = llm.complete("Paul Graham is ") if resp.logprobs is not None: print("\nLog Probabilities:") for lp_list in resp.logprobs: for lp in lp_list: print(f"Token: {lp.token}, LogProb: {lp.logprob}") if "model_version" in resp.additional_kwargs: print("\nModel Version:") print(resp.additional_kwargs["model_version"]) if "raw_completion" in resp.additional_kwargs: print("\nRaw Completion:") print(resp.additional_kwargs["raw_completion"])
```
Log Probabilities:
...
from llama_index.llms.alephalpha import AlephAlpha llm = AlephAlpha(model="luminous-base-control") resp = await llm.acomplete("Paul Graham is ")
In [ ]:
Copied!
```
print(resp)
